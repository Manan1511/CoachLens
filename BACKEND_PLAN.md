...
Prompted by the actual field workflow: one coach, one phone, several different bowlers taking turns in the nets. See `MOBILE_PLAN.md` for the mobile app side of this.
...
## Known gaps found while finalizing MOBILE_PLAN.md (2026-09-12)

Surfaced by walking the mobile capture app's first-run path against the real API. Recorded here rather than fixed, so they're decided deliberately.

- [ ] **Baseline chicken-and-egg — product-blocking, and the most serious of these.** `_score` raises `UnknownBaselineError` for an athlete with no confirmed baseline, before `save_delivery`. But a baseline is meant to be computed from 8–10 benchmark deliveries (PRD §4 Layer 2; `confirm_baseline`'s docstring defers the computation to a coach/UI workflow). **There is no API path that persists a good-quality delivery for a baseline-less athlete** — so you need deliveries for a baseline and a baseline for deliveries. Worse, the one thing that *does* persist without a baseline is a `DATA_SUPPRESSED` delivery, since the quality firewall short-circuits before the baseline lookup: for a brand-new bowler the system keeps the unusable deliveries and discards the usable ones. Every newly created player starts here, so it's the first thing a real coach hits. Fix is a benchmark-collection path that measures and persists kinematics without scoring ("measured, not compared" — needs a new status or a nullable verdict). Demo workaround: pre-seeded baselines, which is what `scripts/seed.py` already does. See `MOBILE_PLAN.md` §10.4.
- [ ] **`capture_metadata` is unenforced provenance.** None of `fps`, `pacing_jitter_pct`, `shutter_speed_sec`, `distance_meters`, `tripod_height_meters`, `camera_roll_deg` is read by any decision logic — they're persisted to jsonb and never consulted (`measurement/audit.py` deliberately measures pacing from frame timestamps instead of trusting the self-reported value). That's defensible, but it means the PRD's capture constraints are enforced *nowhere in the backend*: the capture client's setup overlay is the only gate. Worth stating explicitly rather than leaving the PRD's numbers looking like guarantees.
- [ ] **`shutter_speed_sec` should be `float | None`.** A capture client may be unable to read actual exposure duration at all (`react-native-vision-camera` exposes EV bias only, not exposure duration), and the field being required-float forces it to invent a number into a permanent audit record. Making "unknown" expressible is a one-line schema change.
- [ ] **Session date uses the server's timezone.** `get_or_create_session` defaults to `date.today()`, which is UTC on Render. A coach training between 00:00 and 05:30 IST gets a session dated to the previous UTC day, quietly splitting one nets session in two. Unusual hours and a small fix, but it undermines the mobile app's "same `session_id` all day" assumption.
