...
- Also flagged but not yet fixed (lower severity, tracked here rather than silently dropped): `evaluate_delivery` persists the delivery row before checking the baseline exists (leaves an orphaned delivery + confusing 404 if `UnknownBaselineError` fires); `_filtered_or_raw` silently falls back to unfiltered keypoints for short clips with no flag in the response; `get_rolling_history_deltas` fetches an athlete's entire session/delivery ID history on every ingest (unbounded growth over a season).
