...
- [x] **`capture_metadata` unenforced provenance — documented, not a code fix.** None of its six fields is read by any backend decision logic; this is a fact about the system to keep visible (see `MOBILE_PLAN.md` §7), not something to change here — the enforcement point is the capture client's setup overlay, by design.
