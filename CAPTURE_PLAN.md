...
- [ ] Check whether `SharedArrayBuffer`/multi-threaded WASM is available (needs COOP/COEP response headers from whatever serves `apps/web` — confirm the deployment target sets these, since single-threaded WASM fallback could be substantially slower). **Not yet checked** — the readings above were taken without confirming this either way, so the real number could potentially improve further with multi-threading enabled.
- [ ] `delegate: 'CPU'` vs `'GPU'` comparison — flagged above, not yet run
