...
/** Chronological, compact — a date and a status chip per delivery. Nothing
 *  expands until the coach clicks into a delivery's own report page. */
  ...
  return (
    <div className="flex flex-col gap-md">
      {sessions.map((session) => (
        <div key={session.id}>
          <p className="mb-2 text-caption font-bold uppercase tracking-[0.1em] text-ink-dim">
            {formatDate(session.session_date)}
          </p>
          <div className="flex flex-col gap-1.5 border-l border-line pl-md">
